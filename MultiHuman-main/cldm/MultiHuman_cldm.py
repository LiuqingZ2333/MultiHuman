...
from ldm.modules.PMM import PMM
...
class StablePose(LatentDiffusion):
    def __init__(self, gaussian_kernels, control_config=None, alpha=5, contrastive_weight=0.5, temperature=0.1, *args, **kwargs):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        if 'depth' in batch and len(batch['depth']) != 0:
            depth_condition = batch['depth']
            if bs is not None:
                depth_condition = depth_condition[:bs]
            depth_condition = depth_condition.to(self.device)
            depth_condition = einops.rearrange(depth_condition, 'b h w c -> b c h w')
            depth_condition = depth_condition.to(memory_format=torch.contiguous_format).float()
        else:
            depth_condition = torch.zeros_like(pose_condition).to(self.device).to(memory_format=torch.contiguous_format).float()
        ...
        masks.requires_grad = False  # 2 64 64
        
        return x, dict(c_crossattn=[c], pose_condition=[pose_condition], depth_condition=[depth_condition], pose_mask=masks)
    ...
    def apply_model(self, x_noisy, t, cond, global_strength=1, *args, **kwargs):
        ...
        assert cond['depth_condition'][0] != None
        depth_condition = torch.cat(cond['depth_condition'], 1)
        
        pose_control, contrastive_loss = self.stable_pose_adapter(
            x=x_noisy, 
            timesteps=t, 
            context=cond_txt, 
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        model_output, contrastive_loss = self.apply_model(x_noisy, t, cond, **kwargs)
        ...
        loss_dict.update({f'{prefix}/contrastive_loss': contrastive_loss.mean()})
        ...
        loss = self.l_simple_weight * loss.mean() + self.contrastive_weight * contrastive_loss.mean()
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, plot_denoise_rows=False,
        ...
        if 'depth' in batch:
            log["depth_image"] = rearrange(batch['depth'], 'b h w c -> b c h w')
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                samples, z_denoise_row = self.sample_log(cond={"pose_condition": [c_cat], "depth_condition": [d_cat], "c_crossattn": [c]},
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_cat = c_cat
            uc_depth = d_cat  
            uc_full = {"pose_condition": [uc_cat], "depth_condition": [uc_depth], "c_crossattn": [uc_cross]}
            samples_cfg, _ = self.sample_log(cond={"pose_condition": [c_cat], "depth_condition": [d_cat], "c_crossattn": [c]},
                                            batch_size=N, ddim=use_ddim,
                                            ddim_steps=ddim_steps, eta=ddim_eta,
        ...
        for key,image in log.items():
            if key.startswith("samples"):
                ...
                if "depth_image" in log:
                    draw_depth_mask=(torch.sum(log["depth_image"],1)>0.1).unsqueeze(1).repeat((1,3,1,1)).float()
                    depth_image=(1-draw_depth_mask)*image_draw+draw_depth_mask*log["depth_image"]
                    log_add[key.replace("samples","depth_samples")] = depth_image
...
class AttnResBlock(nn.Module):
    def __init__(
        ...
        self.attn_layer = PMM(image_size=64, patch_size=2, dim=512, depth=2, heads=8, mlp_dim=512, 
    ...
    def _forward(self, x, emb, pose_condition, mask):
        ...
        h = h + self.cond_conv(pose_condition)
        h = self.out_layers(h)
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, pose_condition):
        pose_features = self.pre_extractor(pose_condition, None)  # 1 3 512 512 -- 1 128 128 128    
...
class DepthFeatureExtractor(nn.Module):
    ...
    def forward(self, depth_condition):
        depth_features = self.pre_extractor(depth_condition, None)  
        assert len(self.extractors) == len(self.zero_convs)
        
        output_features = []
        for idx in range(len(self.extractors)):
            depth_features = self.extractors[idx](depth_features, None)
            output_features.append(self.zero_convs[idx](depth_features))
        return output_features


class SharedEmbeddingModule(nn.Module):
    def __init__(self, feature_dim, embedding_dim=256, temperature=0.1):
        super().__init__()
        self.pose_projector = nn.Sequential(
            nn.Conv2d(feature_dim, embedding_dim, kernel_size=1),
        ...
        self.pose_attention = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(embedding_dim//2, 1, kernel_size=1),
            nn.Sigmoid()
        )
        ...
        self.instance_aware = nn.Sequential(
        ...
        self.output_projector = nn.Sequential(
            nn.Conv2d(embedding_dim, feature_dim, kernel_size=1),
    ...
    def forward(self, pose_features, depth_features):

        pose_emb = self.pose_projector(pose_features)
        depth_emb = self.depth_projector(depth_features)

        contrastive_loss = self.compute_contrastive_loss(pose_emb, depth_emb)

        pose_attn = self.pose_attention(pose_emb)
        depth_attn = self.depth_attention(depth_emb)
        

        sum_attn = pose_attn + depth_attn + 1e-8 
        pose_weight = pose_attn / sum_attn
        depth_weight = depth_attn / sum_attn
        

        weighted_pose = pose_emb * pose_weight
        weighted_depth = depth_emb * depth_weight
        

        concat_features = torch.cat([weighted_pose, weighted_depth], dim=1)
        instance_features = self.instance_aware(concat_features)
        
        aligned_features = self.output_projector(instance_features)
        
        return aligned_features, contrastive_loss
        
    
    def compute_contrastive_loss(self, pose_emb, depth_emb):

        batch_size = pose_emb.shape[0]
        pose_emb_flat = pose_emb.reshape(batch_size, -1, pose_emb.shape[2]*pose_emb.shape[3])
        depth_emb_flat = depth_emb.reshape(batch_size, -1, depth_emb.shape[2]*depth_emb.shape[3])
        

        pose_emb_flat = pose_emb_flat.permute(0, 2, 1)
        depth_emb_flat = depth_emb_flat.permute(0, 2, 1)
        

        pose_emb_norm = F.normalize(pose_emb_flat, dim=2)
        depth_emb_norm = F.normalize(depth_emb_flat, dim=2)
        

        losses = []
        for i in range(batch_size):

            similarity = torch.matmul(pose_emb_norm[i], depth_emb_norm[i].transpose(0, 1)) / self.temperature
...
class StablePoseAdapter(nn.Module):
    def __init__(
        ...
        self.depth_feature_extractor = DepthFeatureExtractor(cond_channels, inject_channels)

        self.shared_embedding_modules = nn.ModuleList([
            SharedEmbeddingModule(inject_channels[i], embedding_dim, temperature)
            for i in range(len(inject_channels))
        ])
    ...
    def forward(self, x, timesteps, context, pose_condition, depth_condition=None, mask=None, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)
        pose_features = self.feature_extractor(pose_condition.cuda())  # 1 192 64 64

        if depth_condition is not None:
            depth_features = self.depth_feature_extractor(depth_condition.cuda())
            ...
            for i in range(len(pose_features)):
                aligned_feat, contrastive_loss = self.shared_embedding_modules[i](
                    pose_features[i], depth_features[i]
                )
                aligned_features.append(aligned_feat)
                contrastive_losses.append(contrastive_loss)

            features_to_use = aligned_features
            contrastive_loss = sum(contrastive_losses) / len(contrastive_losses)
        else:
            features_to_use = pose_features
        ...
        for layer_idx, (module, zero_conv) in enumerate(zip(self.input_blocks, self.zero_convs)):
            if layer_idx in self.inject_layers:
                h = module(h, emb, context, features_to_use[self.inject_layers.index(layer_idx)], mask)
            else:
