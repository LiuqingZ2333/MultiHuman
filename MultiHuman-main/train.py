...
def save_model_structure(model, save_path):
    with open(save_path, 'w') as f:
        ...
        for name, parameter in model.named_parameters():
            ...
            total_params += params
            f.write(f"{name}: {parameter.shape}, Parameters: {params:,}\n")
...
def main():
    ...
    if not os.path.exists(os.path.join('/data/root/runs', model_name)):
        os.mkdir(os.path.join('/data/root/runs', model_name))
    ...
    if os.path.exists(os.path.join('/data/root/runs', model_name, run_name)) and not opt.resume:
        import time
    ...
    output_path = os.path.join('/data/root/runs', model_name, run_name)
